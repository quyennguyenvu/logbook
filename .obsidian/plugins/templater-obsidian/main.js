...
var TemplaterSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  add_template_folder_setting() {
    new import_obsidian6.Setting(this.containerEl)
  ...
  add_templates_hotkeys_setting() {
    ...
    this.plugin.settings.enabled_templates_hotkeys.forEach(
      (template, index) => {
        const s = new import_obsidian6.Setting(this.containerEl)
          .addSearch((cb) => {
            new FileSuggest(
              cb.inputEl,
              this.plugin,
...
var InternalModuleDate = class extends InternalModule {
  ...
  generate_now() {
    return (format = "YYYY-MM-DD", offset2, reference2, reference_format) => {
      if (
        reference2 &&
        !window.moment(reference2, reference_format).isValid()
      ) {
      ...
      return window
        .moment(reference2, reference_format)
        .add(duration)
        .format(format)
    }
  }
  ...
  generate_cursor_append() {
    return (content) => {
      ...
      if (!active_editor || !active_editor.editor) {
        log_error(
          new TemplaterError("No active editor, can't append to cursor.")
        )
        return
      }
...
var PromptModal = class extends import_obsidian9.Modal {
  ...
  enterCallback(evt) {
    ...
    if (this.multi_line) {
      if (
        import_obsidian9.Platform.isDesktop &&
        evt.key === "Enter" &&
        !evt.shiftKey
      ) {
        this.resolveAndClose(evt)
...
var UserSystemFunctions = class {
  constructor(plugin) {
    this.plugin = plugin
    if (
      import_obsidian11.Platform.isMobileApp ||
      !(app.vault.adapter instanceof import_obsidian11.FileSystemAdapter)
    ) {
      this.cwd = ""
    } else {
...
var Templater = class {
  ...
  async create_new_note_from_template(
    ...
    const extension =
      template instanceof import_obsidian12.TFile
        ? template.extension || "md"
        : "md"
    const created_note = await errorWrapper(async () => {
      const folderPath =
        folder instanceof import_obsidian12.TFolder ? folder.path : folder
      const path2 = app.vault.getAvailablePath(
        (0, import_obsidian12.normalizePath)(
          `${folderPath ?? ""}/${filename || "Untitled"}`
        ),
        extension
      )
  ...
  overwrite_active_file_commands() {
    ...
    if (!active_editor || !active_editor.file) {
      log_error(
        new TemplaterError("Active editor is null, can't overwrite content")
      )
      return
    }
  ...
  static async on_file_creation(templater, file) {
    ...
    const template_folder = (0, import_obsidian12.normalizePath)(
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("file-menu", (menu, file) => {
        if (file instanceof import_obsidian13.TFolder) {
          menu.addItem((item) => {
            item
              .setTitle("Create new note from template")
...
var CursorJumper = class {
  ...
  async jump_to_next_cursor_location() {
    ...
    if (positions) {
      const fold_info =
        active_editor instanceof import_obsidian14.MarkdownView
  ...
  web: {
    ...
    functions: {
      ...
      random_picture: {
        ...
        examples: [
          { name: "Random picture", example: "<% tp.web.random_picture() %>" },
          {
      ...
      function proceed(type2, value) {
        if (sep ? sep.indexOf(type2) > -1 : type2 == ",") {
          ...
          return cont(function (type3, value2) {
            if (type3 == end2 || value2 == end2) return pass()
            return pass(what)
          }, proceed)
        }
        if (type2 == end2 || value == end2) return cont()
...
var Editor2 = class {
  ...
  desktopShouldHighlight() {
    return (
      import_obsidian16.Platform.isDesktopApp &&
      this.plugin.settings.syntax_highlighting
    )
  }
  mobileShouldHighlight() {
    return (
      import_obsidian16.Platform.isMobileApp &&
      this.plugin.settings.syntax_highlighting_mobile
    )
  }
  async setup() {
    ...
    this.templaterLanguage = import_state.Prec.high(
